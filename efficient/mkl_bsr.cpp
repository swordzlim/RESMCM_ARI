...
public:
    ...
    // sparse_matrix_t s_mtx_csr;
    sparse_matrix_t s_mtx_bsr;
    
    L_MKL_BSR(){
    }

    L_MKL_BSR(sparse_matrix_t s_mtx_bsr){
        this->s_mtx_bsr = s_mtx_bsr;
    }

    L_MKL_BSR(vertex n, edge nnz){
        setN_NNZ(n, nnz);
    }
    ...
    tuple<vertex, vertex> get_N_NNZ(){
        ...
        sparse_status_t status = mkl_sparse_convert_csr_64(this->s_mtx_bsr, SPARSE_OPERATION_NON_TRANSPOSE, &(s_mtx_csr));

        status = mkl_sparse_d_export_csr_64(s_mtx_csr, &indexing, &nrows, &ncols, &csr_row_start, &csr_row_end, &csr_col_indx, &csr_values);

        // sparse_status_t status = mkl_sparse_d_export_bsr_64(this->s_mtx_bsr, &indexing, &layout, &nrows, &ncols, &block_size, &csr_row_start, &csr_row_end, &csr_col_indx, &csr_values);
        ...
        mkl_sparse_destroy_64(s_mtx_csr);

        return tuple<vertex, vertex>(nrows, nnz_bsr);
    }
    ...
    ~L_MKL_BSR(){
        ...
        // mkl_sparse_destroy_64(s_mtx_csr);
        mkl_sparse_destroy_64(s_mtx_bsr);
    }
...
#pragma omp parallel for schedule(dynamic, chunk_size)
    ...
    L_MKL_BSR *to_L_MKL_BSR(){
        L_MKL_BSR *bsr = new L_MKL_BSR();
        vertex new_n = n + (bsr->block_size -  (n % bsr->block_size));
        // vertex new_n = n;
        bsr->setN_NNZ(new_n, m);
        bsr->row_ptrs[0] = 0;
        ...
        for(vertex u_id = 0; u_id < n; u_id ++){
            if(rows[u_id] != nullptr){
                bsr->row_ptrs[u_id + 1] = bsr->row_ptrs[u_id] + rows[u_id]->size();
            } else{
                bsr->row_ptrs[u_id + 1] = bsr->row_ptrs[u_id];
            }
        }

        for(vertex u_id = n + 1; u_id <= new_n; u_id ++){
                bsr->row_ptrs[u_id] = bsr->row_ptrs[n];
        }
        ...
        for(vertex u_id = 0; u_id < n; u_id ++){
            if(rows[u_id] != nullptr){
                for(vertex v_idx = 0; v_idx < rows[u_id]->size(); v_idx ++){
                    bsr->col_ind[cnt] = rows[u_id]->at(v_idx);
                    bsr->values[cnt] = 1;
                    cnt ++;
                }
            }
        ...
        sparse_status_t status = mkl_sparse_d_create_csr_64(&(s_mtx_csr), SPARSE_INDEX_BASE_ZERO, new_n, new_n,
                            bsr->row_ptrs, bsr->row_ptrs + 1, bsr->col_ind, bsr->values);
        // status = mkl_sparse_order_64(bsr->s_mtx_csr);
        status = mkl_sparse_convert_bsr_64(s_mtx_csr, bsr->block_size, SPARSE_LAYOUT_ROW_MAJOR, SPARSE_OPERATION_NON_TRANSPOSE, &(bsr->s_mtx_bsr));
        
        mkl_sparse_destroy_64(s_mtx_csr);

        // mkl_sparse_d_create_bsr_64(&bsr->s_mtx, SPARSE_INDEX_BASE_ZERO, SPARSE_LAYOUT_ROW_MAJOR, new_n, new_n, bsr->block_size,
                                //    bsr->row_ptrs, bsr->row_ptrs + 1, bsr->col_ind, bsr->values);
...
// n: # of vertex
// m: # of edge
void SSP_dynamic(vector<HIN_nb> **HIN, L_MKL_BSR *&graph,
    ...
    if(path->pathLen == 1){
        graph = t_matrices[0]->to_L_MKL_BSR();
        delete t_matrices[0];
        free(t_matrices);
        return;
    }
    ...
    L_MKL_BSR **matrices = (L_MKL_BSR **)malloc(p_l * sizeof(L_MKL_BSR *));
    
    gettimeofday(&end, NULL);
    smcm_time = ((end.tv_sec - start.tv_sec) + (double)(end.tv_usec - start.tv_usec)/1000000.0);

    for(int i = 0; i < p_l; i ++){
        matrices[i] = t_matrices[i]->to_L_MKL_BSR();
...
{   
    ...
    vector<L_MKL_BSR *> tmp_mtcs;
    tmp_mtcs.reserve(p_l);

    L_MKL_BSR *tmp_mtx = nullptr;
    ...
    for (auto it = chain_order.begin(); it != chain_order.end(); ++it) {
        ...
        if(k >= 0 && l >= 0){ // both left mtx and right mtx not in queue 
            ...
            sparse_status_t status = mkl_sparse_spmm_64(SPARSE_OPERATION_NON_TRANSPOSE, matrices[k]->s_mtx_bsr, matrices[l]->s_mtx_bsr, &res_s_mtx);
            // sparse_status_t status = mkl_sparse_spmm_64(SPARSE_OPERATION_NON_TRANSPOSE, matrices[k]->s_mtx, matrices[k]->s_mtx, &res_s_mtx);
            // } catch (const std::exception& e) {
            //     std::cerr << "Caught exception: " << e.what() << std::endl;
            // }

            L_MKL_BSR *res = new L_MKL_BSR(res_s_mtx);

            // tuple<vertex, vertex> t = res->get_N_NNZ();

            // L_MKL_BSR *res = SpGEMM(matrices[k], matrices[l], bins[p_vts[k]], is_existed, n);
            ...
            sparse_matrix_t res_s_mtx;
            sparse_status_t status = mkl_sparse_spmm_64(SPARSE_OPERATION_NON_TRANSPOSE, tmp_mtcs[tmp_size - 1]->s_mtx_bsr, matrices[l]->s_mtx_bsr, &res_s_mtx);
            tmp_mtcs[tmp_size - 1] = new L_MKL_BSR(res_s_mtx);
            ...
            sparse_matrix_t res_s_mtx;
            sparse_status_t status = mkl_sparse_spmm_64(SPARSE_OPERATION_NON_TRANSPOSE, matrices[k]->s_mtx_bsr, tmp_mtcs[tmp_size - 1]->s_mtx_bsr, &res_s_mtx);
            tmp_mtcs[tmp_size - 1] = new L_MKL_BSR(res_s_mtx);
            ...
            sparse_matrix_t res_s_mtx;
            sparse_status_t status = mkl_sparse_spmm_64(SPARSE_OPERATION_NON_TRANSPOSE, tmp_mtx->s_mtx_bsr, tmp_mtcs[tmp_size - 1]->s_mtx_bsr, &res_s_mtx);
            tmp_mtcs[tmp_size - 2] = new L_MKL_BSR(res_s_mtx);
...
// n: # of vertex
// m: # of edge
void build_pair(vector<HIN_nb> **HIN, L_MKL_BSR *&graph, 
                vertex_type *vertex_types, edge_type *edge_types,
                vector<vertex> **bins, vertex *dims,
                MetaPath *path, vertex n, edge m){
...
int main(int argc, char **argv){
    ...
    string input_graph = "/home/chunxu/data/HINTrussICDE2020/DBPedia/graph.txt";
    string input_vertex = "/home/chunxu/data/HINTrussICDE2020/DBPedia/vertex.txt";
    string input_edge = "/home/chunxu/data/HINTrussICDE2020/DBPedia/edge.txt";
    string input_meta_path = "/home/chunxu/cxx/RoseMM/exp_len/materials/DBPedia/input/p_l" + to_string(path_length) + "/DBPedia" + to_string(path_length) + "mkl_bsr.txt";
    string output_file = "/home/chunxu/cxx/RoseMM/exp_len/materials/DBPedia/dp_exp/p" + to_string(thread_num) + "/p_l"+  to_string(path_length) +"/mkl_bsr" + to_string(thread_num) + ".txt";
