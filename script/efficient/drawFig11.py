...
def get_data(ds):
    ...
    for alg in algs:
        array2d.append(list())
        for l in x:
            print(eff_fileroot + alg + '_eff-l' + str(l) + '-t64.txt')
            lst = readFromTXT(eff_fileroot + alg + '_eff-l' + str(l) + '-t64.txt')
            if len(lst) == 0:
                array2d[-1].append(None)
                continue
            sum = 0
            for item in lst:
                sum += item
...
fig.text(0.5, 0.05, 'Fig. 11. Effect of the length of matrix chain', ha='center', fontsize=28)


root = os.path.abspath(".")
plt.savefig(root + '/../../materials/output/fig/fig_11_matrix_chain_length_effect.png', dpi=300)